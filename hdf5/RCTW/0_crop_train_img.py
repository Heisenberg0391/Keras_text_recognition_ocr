...
def hist_equalization(input_img):
    """
    # 灰度直方图均衡
    :param input_img: ndarray-like grayscale img
    :return: ndarray-like grayscale img
    """
    ...
    # find the minimum histogram value (excluding 0)
    cdf_m = np.ma.masked_equal(cdf, 0)
    cdf_m = (cdf_m - cdf_m.min()) * 255 / (cdf_m.max() - cdf_m.min())
    cdf = np.ma.filled(cdf_m, 0).astype('uint8')
    # Now we have the look-up table that gives us the information
    # on what is the output pixel value for every input pixel value.
    # So we just apply the transform.
    output = cdf[input_img]
    return output

def save_mapping_file(label_id_seq, save_dir, img_name, max_label_len):
    '''
    # 将图像标注映射表保存
    ...
    file_path = os.path.join(save_dir, 'rctw_list.txt')
...
if __name__ == '__main__':
    # 路径
    issue_img = 'F:\deeplearning\datasets\文本识别数据集\\rctw17\\train\part1\image_1.jpg'
    img_dir = 'F:\deeplearning\datasets\文本识别数据集\\rctw17\\train\part'
    save_dir = 'F:\deeplearning\datasets\文本识别数据集\\rctw17/color_crop_train'
    ...
    images = []
    for i in range(1, 6):
        img_paths = os.path.join(img_dir+str(i), "*.jpg")
        # 获取全部图像路径，存成list
        images.append(glob.glob(img_paths))
    ...
    for part in images:
        for each in part:
            ...
            w = image.shape[1]
            # 打开标注文件
            with codecs.open(annotation, mode='r', encoding='utf-8-sig') as f:
                for i, row in enumerate(f):
                    ...
                    # 保存截取区的灰度图
                    # gray_cropped = cropped[:,:,:]  # 初始化灰度图输出矩阵
                    # cv2.decolor(cropped, gray_cropped.copy())  # opencv以指针方式直接修改
                    # 二值化
                    # ret2, th2 = cv2.threshold(gray_cropped, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                    # 去除低对比度的
                    # if exposure.is_low_contrast(gray_cropped, fraction_threshold=0.06):
                        # 参数越大表示低对比度阈值越高，即被筛掉的图片对比度越高
                        # Image.fromarray(gray_cropped).show()
                        # flag = False  # fixme 非灰度图不考虑
                        # continue
                    # gray_cropped = rgb_2_gray(cropped)
                    # gray_cropped = hist_equalization(gray_cropped)
                    if flag:
                        ...
                        valid += 1
                        # todo 标注文件写在一个txt里
                        save_mapping_file(label_id_seq, save_dir, img_name, max_label_len)

    print("共{}个，保留{}个".format(total, valid))
    # print('\navg_psnr:{}'.format(np.mean(psnr)))
    # print('\navg_ssim:{}'.format(np.mean(ssim)))