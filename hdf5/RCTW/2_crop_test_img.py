...
def add_noise(raw_img):
    ...
    mode = 2
    if mode == 0:  #不加噪
        cv2.normalize(raw_img, raw_img, 0, 255, cv2.NORM_MINMAX, dtype=-1)  # 归一化
        noisy = raw_img.astype(np.uint8)
    elif mode == 1:  # 瑞利噪声
        h = raw_img.shape[0]
        w = raw_img.shape[1]
        lim = 400
        ...
        noise = noise / noise.max() *310 # 控制分布的灰度范围
...
def save_mapping_file(label_id_seq, save_dir, img_name, max_label_len):
    ...
    file_path = os.path.join(save_dir, 'map_list.txt')
...
if __name__ == '__main__':
    ...
    img_dir = 'F:\deeplearning\datasets\文本识别数据集\\rctw17\\test'
    save_dir = 'F:\deeplearning\datasets\文本识别数据集\\rctw17\\noisy_color_crop_test'
